...
class BaseTrainer:
            ...
            if self.monitor_mode != "off":  # Then there is a metric to monitor
                if self.monitor_metric in log:  # Then we have measured it in this epoch
                    ...
                    if (
                        ...
                        and log[self.monitor_metric] < self.monitor_best
                    ) or (
                        self.monitor_mode == "max"
                        and log[self.monitor_metric] > self.monitor_best
                    ):
                        self.logger.info(
                            f"{self.monitor_metric} Metric improved: {self.monitor_best} -> {log[self.monitor_metric]}. Saving the best model"
                        )
                        self.save_model(
                            path=os.path.join(self.checkpoint_dir, "best_val_model.pth")
                        )
                    ...
                    if self.not_improved_count >= self.early_stop:
                        self.logger.info(
                            f"{self.monitor_metric} Metric did not improve for {self.early_stop} steps. "
                            f"Best value was {self.monitor_best}. Terminating training at epoch {self.current_epoch}"
                        )
                        break
                    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                    ...
                    self.logger.warning(
                        f"Warning: At epoch {self.current_epoch} Metric '{self.monitor_metric}' wasn't measured. Not monitoring it for this epoch."
                    )
    ...
    def resume_checkpoint(self, resume_path=None):
        ...
        if (
            checkpoint["config"]["lr_scheduler"]["type"]
            != self.config["lr_scheduler"]["type"]
        ):
            self.logger.warning(
                "Warning: lr_scheduler type given in config file is different from that of checkpoint. "
