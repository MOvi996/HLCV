...
class TwoLayerNetv1(object):
    ...
    def forward(self, X):
        ...
        # a2 = relu(W1X + b1)
        z2 = np.matmul(X, W1) + b1[np.newaxis, :]
        a2 = np.maximum(0, z2)

        self.params['z2'] = z2
        self.params['a2'] = a2

        # f(X,W,B) = softmax(W2a2 + b2)
        z3 = np.matmul(a2, W2) + b2[np.newaxis, :]
        self.params['z3'] = z3

        scores = np.exp(z3)/(1e-12 + np.sum(np.exp(z3), axis=1)[:,np.newaxis])       #epsilon 1e-12 to avoid zero-division
...
class TwoLayerNetv2(TwoLayerNetv1):
    ...
    def compute_loss(self, X, y=None, reg=0.0):
        ...
        loss = loss/len(y) + reg * (np.linalg.norm(W1)**2 + np.linalg.norm(W2)**2)
...
class TwoLayerNetv3(TwoLayerNetv2):
    ...
    def back_propagation(self, X, y=None, reg=0.0):
        ...
        # f(X,W,B) = softmax(W2a2 + b2)
        # z3 = np.matmul(a2, W2) + b2[np.newaxis, :]
        # scores = np.exp(z3)/np.sum(np.exp(z3), axis=1)[:,np.newaxis]
        ...
        da2 = np.where(a2 > 0, 1, 0)
        
        grad_z3 = (1/N) * (scores - delta)
        #print(grad_z3)
        grads['W2'] = np.matmul(grad_z3.T, a2).T + 2 * reg * W2
        grads['b2'] = np.sum(grad_z3, axis=0)
        
        grad_a2 = np.matmul(grad_z3, W2.T)
        grad_z2 = grad_a2 * da2
        
        grads['W1'] = np.matmul(X.T, grad_z2) + 2 * reg * W1
