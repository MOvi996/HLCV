...
class EncoderBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        result = x
        attention_weights = _ # Needed only if self.need_weights is True for this specific Block
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            ...
            layers[f"encoder_layer_{i}"] = EncoderBlock(num_heads, hidden_dim, mlp_dim, 
                                                        norm_layer, need_weights = i == (num_layers-1)) # Replace None with Block
            #################################
